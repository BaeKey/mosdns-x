...
type cacheDumpFile struct {
	Version int
	Items   []mem_cache.DumpedElem
}
...
type cachePlugin struct {
	*coremain.BP
	...
	ipTagTotal     prometheus.Counter
	defaultTagTotal prometheus.Counter
}
...
func newCachePlugin(bp *coremain.BP, args *Args) (*cachePlugin, error) {
	...
	if len(args.Redis) != 0 {
		...
		mc = mem_cache.NewMemCache(args.Size, 0)
		c = mc
		if len(args.DumpFile) > 0 {
			if n, err := loadCacheDump(args.DumpFile, mc); err != nil {
				bp.L().Warn("failed to load cache dump", zap.String("file", args.DumpFile), zap.Error(err))
			} else {
				bp.L().Info("cache dump loaded", zap.String("file", args.DumpFile), zap.Int("records", n))
			}
		}
	}
	...
	p := &cachePlugin{
		...
		ipTagTotal: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "key_gen_ip_tag_total",
			Help: "The total number of cache keys generated by IP tag",
		}),
		defaultTagTotal: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "key_gen_default_total",
			Help: "The total number of cache keys generated by default logic",
		}),
	}
	bp.GetMetricsReg().MustRegister(p.queryTotal, p.hitTotal, p.lazyHitTotal, p.size, p.ipTagTotal, p.defaultTagTotal)
	return p, nil
}
...
func loadCacheDump(file string, c *mem_cache.MemCache) (int, error) {
	f, err := os.Open(file)
	if err != nil {
	...
	var dump cacheDumpFile
	if err := gob.NewDecoder(f).Decode(&dump); err != nil {
		return 0, err
	}
	if dump.Version != cacheDumpVersion {
		return 0, fmt.Errorf("unsupported cache dump version %d", dump.Version)
	}

	loaded := 0
	now := time.Now()
	for _, item := range dump.Items {
		if item.Key == "" || item.Value == nil || item.ExpirationTime.Before(now) {
			continue
		}
		c.Store(item.Key, item.Value, item.StoredTime, item.ExpirationTime)
		loaded++
	}
	return loaded, nil
}

func saveCacheDump(file string, c *mem_cache.MemCache) (int, error) {
	items := c.Dump()
	dump := cacheDumpFile{
...
func (c *cachePlugin) Close() error {
	if c.memBackend != nil && len(c.args.DumpFile) > 0 {
		if n, err := saveCacheDump(c.args.DumpFile, c.memBackend); err != nil {
			c.L().Warn("failed to save cache dump", zap.String("file", c.args.DumpFile), zap.Error(err))
		} else {
			c.L().Info("cache dump saved", zap.String("file", c.args.DumpFile), zap.Int("records", n))
		}
	}
